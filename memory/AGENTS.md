...
Also run `search_memory(task description)` at session start — category filters are listed per task type.

- multi-step task (feature | bug | refactor): check `.agents/routines/INDEX.md` first
- new-feature:           `.agents/context/principles/distilled/` + `.agents/context/principles/` + `.agents/context/architecture.md` + `.agents/context/domain.md` · mem0: category:[pattern, constraint, integration]
- bug-fix:               `.agents/context/principles/distilled/` + `.agents/context/architecture.md` · mem0: category:[gotcha, resolution, constraint]
- refactor/tests/CI:     `.agents/context/principles/distilled/` · mem0: category:[pattern, constraint]
...
- CONSULT MEMORY: At session start, run `search_memory` with your task description using the category filter for your task type. Pull full content for confirmed-confidence results.
- CAPTURE TO MEMORY: After any task that produced a new pattern, gotcha, or resolution, follow `.agents/playbooks/memory-write.md`.
...
- `.agents/playbooks/memory-write.md` — memory write protocol (follow before calling add_memory)
